...
void ddDRcalo::DRconstructor::implementTowers(xml_comp_t& x_theta, dd4hep::DDSegmentation::DRparamBase* param) {
  ...
  std::cout<<"entering DRconstructor"<<std::endl;


  double currentTheta = x_theta.theta();
  std::cout<<"  currentTheta is "<<currentTheta<<std::endl;
  int towerNo = x_theta.start();
  std::cout<<"  towerNo is "<<towerNo<<std::endl;
  for (xml_coll_t x_dThetaColl(x_theta,_U(deltatheta)); x_dThetaColl; ++x_dThetaColl, ++towerNo ) {
    std::cout<<"  now towerNo is "<<towerNo<<std::endl;
...
It has 11 parameters: the half length in z, the polar angles from the centre of the face at low z to that at high z, H1 the half length in y at low z, LB1 the half length in x at low z and y low edge, LB2 the half length in x at low z and y high edge, TH1 the angle w.r.t. the y axis from the centre of low y edge to the centre of the high y edge, and H2, LB2, LH2, TH2, the corresponding quantities at high zIt has 11 parameters: the half length in z, the polar angles from the centre of the face at low z to that at high z, H1 the half length in y at low z, LB1 the half length in x at low z and y low edge, LB2 the half length in x at low z and y high edge, TH1 the angle w.r.t. the y axis from the centre of low y edge to the centre of the high y edge, and H2, LB2, LH2, TH2, the corresponding quantities at high z
    */
    ...
    std::cout<<"   tower parameters "<<x_theta.height()<<" "<<param->GetH1()<<" "<<param->GetBl1()<<" "<<param->GetTl1()<<" "<<param->GetH2()<<" "<<param->GetBl2()<<" "<<param->GetTl2()<<std::endl;
  ...
  std::cout<<" total number of towers is "<< towerNo - x_theta.start() <<std::endl;
}
...
void ddDRcalo::DRconstructor::implementFibers(xml_comp_t& x_theta, dd4hep::Volume& towerVol, dd4hep::Trap& trap, dd4hep::DDSegmentation::DRparamBase* param) {
  std::cout<<" fiber clasC coreC coreSare "<<fX_cladC.rmax()<<" "<<fX_coreC.rmin()<<" "<<fX_coreS.rmin()<<std::endl;
  ...
  std::cout<<" sipmSize is "<<sipmSize<<std::endl;
  ...
  std::cout<<"   number of fibers in x direction "<<fNumx<<" "<<param->GetTl2()*2<<" "<<sipmSize<<" "<<gridSize<<std::endl;
  fNumy = static_cast<int>( std::floor( ( param->GetH2()*2. - sipmSize )/gridSize ) ) + 1; // in eta direction
  std::cout<<"   number of fibers in y direction "<<fNumy<<std::endl;
  int numxBl2 = static_cast<int>( std::floor( ( param->GetBl2()*2. - sipmSize )/gridSize ) ) + 1; // only used for estimating normals
  std::cout<<"    some parameters called numxBl2 "<<numxBl2<<std::endl;
